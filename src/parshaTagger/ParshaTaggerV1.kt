...
var timesInvoked = 0
//fully functional using a different list for each parsha
fun main() {
...
//TODO make sure to sanitize input!! (i.e. .toLowercase.Capitalize()
    ...
    val listOfAllActualSpellings = listOf(actualSpelling1, actualSpelling2, actualSpelling36)

    val listOfAllPossibleSpellings = listOf(listOfPossibleSpellings1, listOfPossibleSpellings2, listOfPossibleSpellings36)

    val correctSpelling = listOfAllPossibleSpellings.associateWith { listOfAllActualSpellings }

    var parshaName = ""
    listOfAllPossibleSpellings.forEach {
        for (word in shiurTitle.split(" ", "-", "_")) if (/*correctSpelling.containsKey(word)*//*<--not tested yet; tested version-->*/ it.indexOf(word) != -1) {
//            parshaName = correctSpelling[word].toString()
        }
    }
}
 /*   println(parshaName)
