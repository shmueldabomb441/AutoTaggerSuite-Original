...
//TODO add spellings like B'Shalach(Capital letter before and after apostrophe) to listOfPossibleSpellings
...
fun main() {
    ...
    if(speaker=="") TODO("There was no speaker given. Make sure to implement a message saying so.")
    processShiurim@ for (file in listOfMp3s) {
        ...
        if (getParshaDontPrint != null && getParshaDontPrint != "") tag.setField(FieldKey.ALBUM, getParsha(title, dontPrintToConsole = false))
        else {
        ...
        checkForDate@ while (true) {
            ...
            if (numberOfMatchesInTitle == 0 && yearGivenInFile) break@checkForDate
            else if (numberOfMatchesInTitle == 0 && !yearGivenInFile) {rejects[title] = "No Date Given In File Nor In Title.";continue@processShiurim}
            else {
                ...
                if (numberOfMatchesInTitle == 1 && !yearGivenInFile) year = yearInTitle
                    ...
                    rejects[title] = "Different Year In Title Than In File. Unable To Determine Correct Year."
                    continue@processShiurim
                } else if (numberOfMatchesInTitle > 1) {
                    /*there are 2+ dates in title*/
                    rejects[title] = "Two Years In Title. Unable To Determine Correct Year."
                    continue@processShiurim
                }
            }
            break@checkForDate
        }
        if (year != "") {tag.setField(FieldKey.CONDUCTOR, "Parsha $year")
        tag.setField(FieldKey.YEAR, year)} else TODO("Year still equals \"\". Unacceptable. I just tried to set the year of the shiur and the year was blank.")
    ...
    for ((listOfPossibleSpellings, mapOfPossibleSpellingToCorrectSpelling) in mapOfPossibleSpellingsListToMapOfPossibleSpellingToCorrectSpelling1) {
        for (spelling in listOfPossibleSpellings) {
            if (capitalizedShiurTitle.contains(spelling)/*if there is a recognized parsha name in the title*/) {
                ...
                val secondParshaOrNull = containsSecondParsha(capitalizedShiurTitle, listOfPossibleSpellings)
                if (secondParshaOrNull != null/*there is a second parsha*/) {
                    if (actualSpelling in doubleParshiyosList && secondParshaOrNull in doubleParshiyosList) {
                        ...
                        rejects[givenShiurTitle] = "Two Parshiyos In Title. Unable To Determine Proper Category."
                        break@outerLoop
                    }
...
//                properParshaName = mapOfPossibleSpellingToCorrectSpelling[spelling].toString()
                if (!dontPrintToConsole && secondParshaOrNull == null) {
